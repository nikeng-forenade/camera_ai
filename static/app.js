...
(function () {
    ...
    if (showZones) {
      zones.forEach((poly, zi) => {
        ...
        if (svg) {
          ...
          label.setAttribute("transform", "translate(" + Math.min(94, Math.max(4, minX + 3.5)) + " " + Math.min(96, Math.max(3, minY + 2.8)) + ")");
          const plate = document.createElementNS("http://www.w3.org/2000/svg", "rect");
          plate.setAttribute("x", "-3.5");
          plate.setAttribute("y", "-2.8");
          plate.setAttribute("width", "8");
          plate.setAttribute("height", "5.6");
          plate.setAttribute("rx", "1.6");
          const text = document.createElementNS("http://www.w3.org/2000/svg", "text");
          text.setAttribute("y", "0.9");
  ...
  function drawRectPreview(sx, sy, cx, cy) {
    ...
    label.setAttribute("transform", "translate(" + Math.min(94, Math.max(4, (Math.min(sx, cx) * 100) + 3.5)) + " " + Math.min(96, Math.max(3, (Math.min(sy, cy) * 100) + 2.8)) + ")");
    const plate = document.createElementNS("http://www.w3.org/2000/svg", "rect");
    plate.setAttribute("x", "-3.5"); plate.setAttribute("y", "-2.8"); plate.setAttribute("width", "8"); plate.setAttribute("height", "5.6"); plate.setAttribute("rx", "1.6");
    const text = document.createElementNS("http://www.w3.org/2000/svg", "text");
    text.setAttribute("y", "0.9"); text.textContent = "ZON " + (zones.length + 1);
    label.append(plate, text);
    svg.appendChild(label);
  }
  function clearDraft() {

...
- **Kontinuerlig live (v0.7)** — aktiverade kameror körs på servern dygnet runt (YOLO + HA-event fortsätter) även utan öppen webbläsare.- **Detektionsfilter per kamera (v0.13.3)** — välj själv i **Inställningar → Kameror**: en enkel **vågrät linje** (dra den upp/ner, kolla ovanför/nedanför) eller en **ruta/zon** som du drar ut på bilden och flyttar hörnen på (inuti/utanför). Det som filtreras bort räknas inte, ritas inte och triggar inga HA-event (hela kedjan).- **Stoppa ström ≠ stoppa analys (v0.9)** — ”Stoppa ström” stänger bara videon till GUI:t; kameraworkern, YOLO och HA-event fortsätter köra på servern. (”Starta kamera” startar workern igen om den stoppats helt.) Streamen är **AV vid uppstart** – starta manuellt eller sätt `LIVE_STREAM_AUTOSTART=true` i .env.

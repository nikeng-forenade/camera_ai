...
- **Kontinuerlig live (v0.7)** — aktiverade kameror körs på servern dygnet runt (YOLO + HA-event fortsätter) även utan öppen webbläsare.- **Detektionszon per kamera (v0.13)** — rita själv en **zon med punkter/hörn** direkt på bilden (Inställningar → Kameror): klicka för att lägga hörn, dra för att flytta, dubbelklicka för att ta bort. Välj om det som är **inuti** eller **utanför** zonen ska kollas. Det utanför räknas inte, ritas inte och triggar inga HA-event (hela kedjan filtreras).- **Stoppa ström ≠ stoppa analys (v0.9)** — ”Stoppa ström” stänger bara videon till GUI:t; kameraworkern, YOLO och HA-event fortsätter köra på servern. (”Starta kamera” startar workern igen om den stoppats helt.) Streamen är **AV vid uppstart** – starta manuellt eller sätt `LIVE_STREAM_AUTOSTART=true` i .env.

...
- **Kontinuerlig live (v0.7)** — aktiverade kameror körs på servern dygnet runt (YOLO + HA-event fortsätter) även utan öppen webbläsare.- **Detektionslinje per kamera (v0.12)** — rita en vågrät linje på bilden i **Inställningar → Kameror** och välj om det som är **ovanför** eller **nedanför** linjen ska kollas. Det på fel sida räknas inte, ritas inte och triggar inga HA-event (hela kedjan filtreras).- **Stoppa ström ≠ stoppa analys (v0.9)** — ”Stoppa ström” stänger bara videon till GUI:t; kameraworkern, YOLO och HA-event fortsätter köra på servern. (”Starta kamera” startar workern igen om den stoppats helt.) Streamen är **AV vid uppstart** – starta manuellt eller sätt `LIVE_STREAM_AUTOSTART=true` i .env.
